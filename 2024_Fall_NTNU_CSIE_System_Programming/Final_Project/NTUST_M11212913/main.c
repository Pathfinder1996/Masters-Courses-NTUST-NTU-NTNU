...
// Register table, pass2用 給跟 Register 有關的指令用
typedef struct {
    char register_name[3];
    char register_number[2];
} REGTABLE;

// object code table, print record用
typedef struct {
    int code_address;
    char code[100];
} OBJTABLE;
...
// pass1
void readMnemonicFile(void);
int pass1(FILE *file_pointer);
void processAsmInput(const char *line_buffer, char *asm_label, char *asm_opc, char *asm_operand);
int splitToken(const char *line_buffer, char token_buffer[][MAX_TOKEN_LEN]);
int isDirectiveOrMnemonic(const char *token);
int addSYMTAB(const char *label, int address);
// pass2 
...
void writeOBJFile();
// 進制轉換 算址 pass2用
int hexToTen(const char *hex_string);
void tenToHex(int value, int width, char *output_string);
void bitsToHex(int a, int b, int c, int d, char *output_hex);
...
void readMnemonicFile(void) {
    ...
    if (!file_in) {
        printf("Cannot open mnemonic.txt\n");
        return;
    }
...
int pass1(FILE *file_pointer) {
    ...
    while (fgets(line_buffer, sizeof(line_buffer), file_pointer)) {
        ...
        processAsmInput(line_buffer, asm_label, asm_opc, asm_operand);
        // printf(line_buffer);
        counter = counter + 1;
        ...
        if (is_first_line) {
            ...
            if (strcmp(asm_opc, "START") == 0) {
                ...
                // program position counter = init(START line)
                LOCCTR = start_address;

                if (strcmp(asm_label, "*") != 0) {
                    strcpy(program_name, asm_label);
                    addSYMTAB(asm_label, LOCCTR);
                }
        ...
        if (strcmp(asm_label, "*") != 0) {
            return_value = addSYMTAB(asm_label, LOCCTR);
        }
        if (return_value == -1) {
...
void processAsmInput(const char *line_buffer, char *asm_label, char *asm_opc, char *asm_operand) {
    ...
    int token_count = splitToken(line_buffer, token_buffer);

    if (token_count == 0) {
        return;
    }

    if (isDirectiveOrMnemonic(token_buffer[0])) {
...
void pass2() {
    ...
    for (i = 0; i < asmtable_size; i++) {
        ...
        if (format_number == 2) {
            format2Process(i, pass2_object_code);
        } 
...
void readRegisterFile(void) {
    ...
    if (!file_in) {
        printf("Cannot open registers.txt\n");
        return;
    }
...
void format4Process(int index, char *obj_code) {
    ...
    int base_opcode_value = hexToTen(base_opcode);
    int final_opcode_value = base_opcode_value + f4_case;
    char final_opcode_string[10];
    tenToHex(final_opcode_value, 2, final_opcode_string);

    int target_address = 0;
    int symbol_value = findSymbol(f4_operand);
    ...
    tenToHex(target_address & 0xFFFFF, 5, address_20bits);

    char mid_value[10];
    bitsToHex(x, b, p, e, mid_value);
...
void format3Process(int index, char *obj_code) {
    ...
    int base_opcode_value = hexToTen(opecode_to_hex);
    ...
    int symbol_value = findSymbol(f3_operand);
    ...
    int address_value = hexToTen(address_to_hex) & 0xFFFF;
    char final_disp[10];
    tenToHex(address_value, 3, final_disp);
...
void writeOBJFile() {
    ...
    // Start Address
    tenToHex(start_address, 6, temp);
    printf("%s^", temp);
    fprintf(file_out, "%s^", temp);

    // Program Length
    tenToHex(program_length, 6, temp);
    printf("%s\n", temp);
    fprintf(file_out, "%s\n", temp);
    ...
    while (i < obj_code_size) {
        ...
        tenToHex(record_start, 6, temp);
        printf("%s^", temp);
        fprintf(file_out, "%s^", temp);

        tenToHex(bytes_sum, 2, text_length);
